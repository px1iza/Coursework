...
public class DocumentServiceTests
{
    ...
    [Fact]
    public void SortByAuthor_MultipleDocuments_ReturnsSortedList()
    {
        var service = CreateIsolatedService();
        service.AddDocument(new DocumentBLL { Title = "Книга1", Author = "Яценко" });
        service.AddDocument(new DocumentBLL { Title = "Книга2", Author = "Андрієнко" });

        var result = service.SortByAuthor();

        Assert.Equal("Андрієнко", result[0].Author);
        Assert.Equal("Яценко", result[1].Author);
    }
